...
--when using coalesce, two parameters should have the same data type
...
--e) 내가 원하는 컬럼 추가해보기
select distinct case when gender = 'M' then '남성' when gender = 'F' then '여성' end as gender
from user_info ui 
...
end as age_group, sum(gmv) as gmv
from online_order oo 
left join user_info ui on oo.userid = ui.userid 
group by 1
order by 1
...
end as item_concept, sum(gmv) as gmv
from online_order oo 
join item i on oo.itemid = i.id 
group by 1
order by 2 desc
...
--3. 할인률, 판매가, 이익률 계산하기
-- sum should be added when doing division 
select c.cate1,
...
round(sum(cast(gmv as numeric)) / count(distinct userid), 2) as avg_gmv_per_customer
from online_order oo 
join item i on oo.itemid = i.id
group by 1
order by 4 desc
