...
--추가질문: 카테고리별 매출액은 어떻게 될까? (Join 테이블에 Join을 한번더)
...
select cate1, cate2, cate3, sum(gmv) as gmv
from category c
join online_order oo on oo.itemid = c.id
group by 1,2,3
order by 4 desc
...
from online_order oo
join item i on oo.itemid = i.id
left join category c on i.category_id = c.id -- (and cate1 = '셔츠')는 결과값이 다르
where cate1 = '셔츠'
;

select *
from online_order oo 
left join user_info ui on oo.userid, ui.userid --(on뒤에는 boolean만 와야)
